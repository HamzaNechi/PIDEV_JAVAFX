...
public class InscriptionController implements Initializable {
   ...
   //  public class CryptWithMD5 
     //{ private static MessageDigest md; public static String cryptWithMD5(String pass)
     //{ try { md = MessageDigest.getInstance("MD5");
     //byte[] passBytes = pass.getBytes(); md.reset(); byte[] digested = md.digest(passBytes); 
     //StringBuffer sb = new StringBuffer(); for(int i=0;i<digested.length;i++){ sb.append(Integer.toHexString(0xff & digested[i])); 
     //} return sb.toString(); } c
       //      atch (NoSuchAlgorithmException ex)
         //    { Logger.getLogger(CryptWithMD5.class.getName()).log(Level.SEVERE, null, ex); } return null; } }
            String role = "[\"ROLE_USER\"]";
     //  JSONParser parser = new JSONParser();  
//JSONObject json = (JSONObject) parser.parse(role); 
        ...
        {
            labelImage.setText("veuillez inserer votre photo");
        }  
        else{
            
                 us.insertUserPst(u);
                 us.envoyerMail(email);
                 final FXMLLoader fxmlLoader = new FXMLLoader(getClass().getResource("/view/vitrine/success.fxml"));
                 final Node node;
                    try {
...
try {
          String filename;
            ...
            File f=chooser.getSelectedFile();
                ...
                //Déplacer l'image
               String newPath="C:\\wamp64_3.2\\www\\Soutenance\\web\\FormulaOneWeb_fini\\public\\images\\user\\";
