...
public class EquipeService implements IService<Equipe>{
    ...
    public void delete(int id) {
       ...
       // MembreService ms=new MembreService();
        ...
        if(eq.getEquipe_id() != 0){
            String req = "DELETE FROM equipes WHERE id=?";
            try {
    ...
    public void update(Equipe e) {
        ...
        if(eq.equals(e)){
            String req = "UPDATE equipes SET nom=? ,logo=?, voiture =? , pays_origine = ? WHERE id = ?";
            try {
                pst = conn.prepareStatement(req);
    ...
    public List<Equipe> read() {
        ...
        try {
            ...
            while(rs.next()){
                list.add(new Equipe(rs.getInt("id"),rs.getString("nom"),rs.getString("email"),rs.getString("logo"), rs.getString("voiture"),rs.getString("pays_origine")));
            }
        } catch (SQLException ex) {
            Logger.getLogger(EquipeService.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    public List<Equipe> readWithImageView() {
        ...
        try {
            ...
            while(rs.next()){
                String path="http://127.0.0.1:8000/images/equipe/"+rs.getString("logo");
                ImageView img = new ImageView(new Image(path,true));
                img.setFitHeight(50);
                img.setFitWidth(50);
                list.add(new Equipe(rs.getInt("id"),rs.getString("nom"),rs.getString("logo"),img, rs.getString("voiture"),rs.getString("pays_origine")));
            }
        } catch (SQLException ex) {
            Logger.getLogger(EquipeService.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    public Equipe readById(int id) {
        String req="select * from equipes where id = '"+ id+"'";
    ...
    public void GeneratePdf(String name){
        ...
        String path="C:\\wamp64_3.2\\www\\Soutenance\\web\\FormulaOneWeb_fini\\public\\documents\\";
