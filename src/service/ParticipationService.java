...
public class ParticipationService implements IServiceA<Participation> {
    ...
    public List<Participation> readWinners(int y) {
        String req = "select * from participation p join courses c on p.course_id=c.id join saisons s on s.id=c.saison_id where s.year=" + y + " and position=1";
        List<Participation> list = new ArrayList<>();
    ...
    public List<Participation> search(String query) {
        String req = "select * from participation p "
                ...
                + "join pilotes pi on pi.id=p.pilote_id "
                + "join membres m on m.id=pi.id "
                + "join equipes e on p.equipe_id=e.id where "
                + "m.nom like '%"+query
                + "%' or e.nom like '%"+query
                + "%' or c.nom like '%"+query
                +"%'";
    ...
    public Participation readById(int id) {
        ...
        try {
            ...
            rs = ste.executeQuery(req);
            while (rs.next()) {
                p.setParticipation_id(rs.getInt("id"));
    ...
    public List<Participation> filter(Integer year, Integer course_id, Integer pilote_id, Integer equipe_id) {
        ...
        String req = (" select * from participation p "
                ...
                + " join pilotes pi on pi.id=p.pilote_id "
                + " join membres m on m.id=pi.id "
                + " join saisons s on c.saison_id=s.id "
                + " join equipes e on p.equipe_id=e.id where 1=1 ");

        if (year != null) {
            req = req + (" AND s.year= " + year);
        }
        if (course_id != null) {
    ...
    public List<Participation> readBySaison(int year) {
        String req = "select * from participation p join courses c on p.course_id=c.id where c.saison_year=" + year;
        List<Participation> list = new ArrayList<>();
    ...
    public List<Participation> readBySaison2(int year) {
        String req = "select * from participation p join courses c on p.course_id=c.id where c.saison_year=" + year + "and position=1";
    ...
    public List<Participation> readByCourse(int id) {
       String req = "select * from participation p join courses c on p.course_id=c.id where p.course_id=" + id ;
        System.out.println("requete readByCourse : "+req);
        List<Participation> list = new ArrayList<>();
